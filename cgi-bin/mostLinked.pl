...
# mostLinked.pl
# Generates a table listing which programs have been linked to the most.
...
my $str  = "select redirect.progid, program.name, count(*) as count ";
# $str .= "from program, redirect where redirect.progid = program.ident ";
$str .= " from program join redirect on redirect.progid = program.ident";
$str .= " where program.remdate = '0000-00-00' ";
$str .= "and (date > date_add(now(), interval - 70 day)) ";
$str .= "group by redirect.progid order by count desc limit 10";
...
while (my $prg = $sh->fetchrow_hashref()) {
  my %prg = %$prg;
  my ($progid, $name, $count) = @prg{qw(progid name count)};
