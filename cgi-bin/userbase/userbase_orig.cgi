...
sub load_prefs()
{
	...
	{
		unless((crypt($rht,'Tv') eq 'TvilHoWFlxbAo')) { exists $PREF{chr(114).chr(101).chr(103).chr(101).chr(114).chr(114)} && ref($PREF{chr(114).chr(101).chr(103).chr(101).chr(114).chr(114)}) eq 'CODE' && &{$PREF{chr(114).chr(101).chr(103).chr(101).chr(114).chr(114)}}(); print_http_headers(); exit; }
	}
...
sub showusers
{
	...
	if($PREF{use_builtin_realname_field} =~ /yes/i) { $columns{'04'}{name} = 'ubrealname';	$columns{'04'}{title} = 'Real Name'; }
	if($PREF{use_builtin_email_field} =~ /yes/i)	{ $columns{'05'}{name} = 'ubemail';	$columns{'05'}{title} = 'Email Address'; }
...
sub set_cookie($$$)
{
	...
	{
		if($expiry eq "")	{ $cookie = new CGI::Cookie(-name => $name, -value => $value, -path => '/', -domain => $PREF{cookie_domain});				}
		else			{ $cookie = new CGI::Cookie(-name => $name, -value => $value, -path => '/', -domain => $PREF{cookie_domain}, -expires => $expiry);	}
	}
	else
	{
  ...
  else 
  {
    ...
    if($PREF{smtp_server})
    {
      ...
      eval
      {
        ...
        {
          eval { MIME::Lite->send('smtp', $PREF{smtp_server}, Timeout=>30, Port=>$PREF{smtp_port}, Debug=>0); };
        }
    ...
    {
      if($smtp_error) { enc_warn "$PREF{internal_appname}: send_email(): SMTP failed, so falling back to sendmail.  SMTP error was: $smtp_error\n"; }
    ...
    {
      printd "in blat loop...\n";
      if($sendmail_error) { enc_warn "$PREF{internal_appname}: send_email(): sendmail failed, so trying blat.  sendmail error was: $sendmail_error\n"; }
...
sub start_html_output_std
{
	...
	if($PREF{internal_appname} eq 'userbase' && -e $PREF{default_account_temp_file}) { print qq`<div style="margin: 20px auto; max-width: 400px;">To determine your default admin account, login to your server via your normal method (FTP, etc) and then read the file $PREF{default_account_temp_file} (within your cgi-bin folder or login folder by default).&nbsp; This message will be displayed until you delete that file.</div>\n`; }
}
...
sub finish_html_output_std
{
	...
	elsif($PREF{encodable_app_template_file} && -e $PREF{encodable_app_template_file})
	{
		...
		while(<$infh>)
		{
			if($found_token)					{ print $_; }
			elsif(/%%encodable_app_output%%(?:\s*-->)?(.*?)$/i)	{ print $1; $found_token = 1; }
		}
		close $infh or die "$0: couldn't close \$PREF{encodable_app_template_file} ('$PREF{encodable_app_template_file}') after reading:: $!\n";
	}
	else
...
sub try_to_load_image_modules()
{
	...
	if($tried_im_and_failed   &&   $tried_gd_and_failed)
	{
		...
		foreach my $pref (keys %PREF) { $PREF{$pref} = 'no' if $pref =~ /^try_to_use_(imagemagick|gd)/i; }
	}
}
...
elsif($qs =~ /(?:^|&)format=mini(?:&|$)/)				{ $PREF{member_is_logged_in} ? print_main_menu() : $qs =~ /action=validate/ ? do_login() : prompt_for_login(); }
...
elsif($qs =~ /(?:^|&)pdir=(\d+)&ppge=(.+)(?:&|$)/)			{ display_protected_page_simple($1,$2);		}
...
elsif($qs =~ /do_email_test/ && ($PREF{enable_email_test} =~ /yes/i || $PREF{admin_is_logged_in})) { do_email_test();	}
